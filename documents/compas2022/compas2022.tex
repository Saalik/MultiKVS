...
Our different backend version are impelmented in Java.
All of our versions implement the same interface that we enhanced if needed by the specification.

We created abstract type to handles all our operations:
\begin{itemize}
  \item TransactionID
  \item Value
  \item Timestamp
...
Value encapsualtes the value of an Object-version and is used to compare and merge different Object-versions.
Timestamp is an implementation of time tracking within the system.

Our keyvalue store is implemented in a Multimap of Key to Values and contains for every key all the Object-version that have been committed.

When a transaction is running we create a transaction object with the dependency timestamp, a unique transaction identifier and an empty state of transaction.
